...
* 2 - Session Security, Server Side Session Invalidation upon Sign Out
...
## Part 3
While security tests _could_ be automated for 100+ endpoints, that many endpoints exposed would represent a rather significant and unusual attack surface from an architectural and security standpoint. I'd like to know more about the nature of the endpoints in the description and whether they are exposed or internal behind a gateway or revcerse proxy and secured with mTLS. If the endpoints are all exposed and there is no other option I would first implement Contract Testing (e.g. Pact) to ensure the _Auth_ header is mandatory across all schemas...then perhaps explore Dynamic App Security Testing tools as an additional measure.
